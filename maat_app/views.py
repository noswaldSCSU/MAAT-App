...
def show_instructions(request, participant_id, experiment_id):
    ...
    return render(request, 'maat_app/instructions.html', {'participant': participant, 'instructions': experiment.instructions})
...
def configure_experiment(request, experiment_id):
    experiment = get_object_or_404(Experiment, id=experiment_id)
    if request.method == 'POST':
        form = InstructionsForm(request.POST, request.FILES, instance=experiment)
        if form.is_valid():
            form.save()
...
# Customize Instructions and Settings (Step 2)
@login_required
...
def edit_experiment(request, experiment_id):
    ...
    return render(request, 'maat_app/edit_experiment.html', {'form': form})
